...
func (grq *GroupRoleQuery) sqlAll(ctx context.Context) ([]*GroupRole, error) {
	...
	if grq.withGroups != nil || grq.withUsers != nil {
		withFKs = true
	}
	if withFKs {
	...
	if query := grq.withUsers; query != nil {
		...
		nodeids := make(map[uuid.UUID][]*GroupRole)
		for i := range nodes {
			if fk := nodes[i].user_group_roles; fk != nil {
				ids = append(ids, *fk)
				nodeids[*fk] = append(nodeids[*fk], nodes[i])
			}
		}
		...
		for _, n := range neighbors {
			nodes, ok := nodeids[n.ID]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "user_group_roles" returned %v`, n.ID)
			}
			for i := range nodes {
