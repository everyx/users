...
func (wrc *WorkspaceRoleCreate) check() error {
	...
	if _, ok := wrc.mutation.UsersID(); !ok {
...
func (wrc *WorkspaceRoleCreate) createSpec() (*WorkspaceRole, *sqlgraph.CreateSpec) {
	...
	if value, ok := wrc.mutation.UserID(); ok {
		_spec.Fields = append(_spec.Fields, &sqlgraph.FieldSpec{
			Type:   field.TypeUUID,
			Value:  value,
	...
	if nodes := wrc.mutation.UsersIDs(); len(nodes) > 0 {
		edge := &sqlgraph.EdgeSpec{
			Rel:     sqlgraph.M2O,
			Inverse: true,
