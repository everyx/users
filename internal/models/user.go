...
func (u *User) assignValues(values ...interface{}) error {
	...
	if value, ok := values[2].(*sql.NullBool); !ok {
		return fmt.Errorf("unexpected type %T for field confirmed", values[2])
	} else if value.Valid {
		u.Confirmed = value.Bool
	}
	if value, ok := values[3].(*sql.NullTime); !ok {
		return fmt.Errorf("unexpected type %T for field confirmation_sent_at", values[3])
	} else if value.Valid {
		u.ConfirmationSentAt = new(time.Time)
		*u.ConfirmationSentAt = value.Time
	}
	if value, ok := values[4].(*sql.NullString); !ok {
		return fmt.Errorf("unexpected type %T for field confirmation_token", values[4])
	} else if value.Valid {
		u.ConfirmationToken = new(string)
		*u.ConfirmationToken = value.String
	}
	if value, ok := values[5].(*sql.NullTime); !ok {
		return fmt.Errorf("unexpected type %T for field recovery_sent_at", values[5])
	} else if value.Valid {
		u.RecoverySentAt = new(time.Time)
		*u.RecoverySentAt = value.Time
	}
	if value, ok := values[6].(*sql.NullString); !ok {
		return fmt.Errorf("unexpected type %T for field recovery_token", values[6])
	} else if value.Valid {
