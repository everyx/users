...
func (wrq *WorkspaceRoleQuery) QueryUsers() *UserQuery {
	query := &UserQuery{config: wrq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := wrq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (wrq *WorkspaceRoleQuery) sqlAll(ctx context.Context) ([]*WorkspaceRole, error) {
	...
	if wrq.withWorkspaces != nil || wrq.withUsers != nil {
		withFKs = true
	}
	if withFKs {
	...
	if query := wrq.withUsers; query != nil {
		ids := make([]uuid.UUID, 0, len(nodes))
		nodeids := make(map[uuid.UUID][]*WorkspaceRole)
		for i := range nodes {
			if fk := nodes[i].user_workspace_roles; fk != nil {
				ids = append(ids, *fk)
				nodeids[*fk] = append(nodeids[*fk], nodes[i])
			}
		}
		...
		for _, n := range neighbors {
			nodes, ok := nodeids[n.ID]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "user_workspace_roles" returned %v`, n.ID)
			}
			for i := range nodes {
